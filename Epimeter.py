...
def build_index(protein_fasta, kmer_bounds, index_name, scores_matrix, weights_vector):  
    ...
    scores_matrix: path to file of scores matrix
    weights_vector: path to file of weights vector 
    ...
    peptide_index.set_scores_and_weights(scores_matrix) 
    protein = [] 
...
def query_epitope(epitopes, index_name): 
    ...
    for each epitope per line in a .csv file. also writes a .csv file
    with the correspondung peptides for each nearest neighbor found. 
...
def main(): 
    ...
    index_parser.add_argument('-s', '--scores-matrix', type=str,
                              help="path to blosum scores matrix")
    index_parser.add_argument('-w', '--weights-vector', type=str,
                              help="path to vector weights")
